# Introduction to AI in Azure

## Course Information

- **Instructor/Source**: Microsoft Learn
- **Duration**: Time period
- **Status**: In Progress
- **Start Date**: 2026-01-30
- **Completion Date**: YYYY-MM-DD (if applicable)

## Description

This course introduces fundamentals concepts related to artificial intelligence (AI), and the services in Microsoft Azure that can be used to create AI solutions. The course is not designed to teach students to become professional data scientists or software developers, but rather to build awareness of common AI workloads and the ability to identify Azure services to support them.

## Learning Objectives

- [ ] Understand fundamental AI concepts
- [ ] Identify common AI workloads
- [ ] Recognize Azure services for AI solutions

## Course Structure

### Modules

1. Overview of AI concepts

- [Explore Simple Agent](lectures/[Overview]_explore-simple-agent.md)
- [Responsible AI](lectures/[Overview]_responsible-ai.md)

2. Get started in Microsoft Foundry

- [AI App Components](lectures/ai-app-components.md)
- [Understand Azure](lectures/understand-azure.md)

3. Introduction to machine learning concepts

### Key Concepts

Important concepts covered in this course:

- [Computer Vision](concepts/computer-vision.md)
- [Generative AI](concepts/generative-ai.md)
- [Natural Language Processing](concepts/nlp.md)
- [Optical Character Recognition](concepts/optical-character-recognition.md)
- [Retrieval-Augmented Generation](concepts/rag.md)
- [Speech Recognition](concepts/speech-recognition.md)
- [Speech Synthesis](concepts/speech-synthesis.md)

### Resources

Additional materials and references:

- [Resource links](https://learn.microsoft.com/en-us/training/paths/introduction-to-ai-on-azure/)
- [Certification Information](https://learn.microsoft.com/en-us/credentials/certifications/azure-ai-fundamentals/?practice-assessment-type=certification)

## Progress Tracking

### Completed Topics

- [x] Overview of AI concepts
- [x] Get started in Microsoft Foundry
- [ ] Introduction to machine learning concepts
- [ ] Get started with machine learning in Azure
- [ ] Introduction to generative AI and agents
- [ ] Get started with generative AI and agents in Microsoft Foundry
- [ ] Introduction to text analysis concepts
- [ ] Get started with text analysis in Microsoft Foundry
- [ ] Introduction to AI speech concepts
- [ ] Get started with speech in Microsoft Foundry
- [ ] Introduction to computer vision concepts
- [ ] Get started with computer vision in Microsoft Foundry
- [ ] Introduction to AI-powered information extraction concepts
- [ ] Get started with AI-powered information extraction in Microsoft Foundry

## Notes & Reflections

Overall thoughts about the course, key takeaways, or areas for review.
